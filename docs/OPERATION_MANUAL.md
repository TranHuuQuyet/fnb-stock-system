...
17. Vào `Control > Ca làm việc` để khởi tạo bảng chấm công tháng nếu cần sắp ca và theo dõi giờ công
18. Vào `Admin > Config` để chỉnh:
   - `allowFifoBypass`
   - `anomalyThreshold`
...
2. Vào `Scan Logs` để xem chi tiết thao tác, warning/error và các phiếu chuyển kho
3. Nếu có phiếu chuyển kho đang đến chi nhánh mình, vào tab `Chuyển kho` để xác nhận số lượng thực nhận
4. Nếu nhận thiếu hoặc hỏng, nhập đúng `Số lượng nhận` và bắt buộc ghi chú lý do chênh lệch
5. Vào `Control > Kho nguyên liệu` để:
...
6. Vào `Control > Ca làm việc` để xem bảng chấm công tháng, giờ thử việc/chính thức, phụ cấp, đi trễ/về sớm và in bảng lương nếu cần
7. Lưu ý: route `/dashboard` hiện chỉ redirect tương thích, không còn là màn vận hành chính của manager.
...
11. `Admin > Recipes`, `/dashboard` và các màn POS/anomaly không còn là bề mặt web vận hành chính; backend API vẫn còn để tích hợp hoặc mở lại sau.
...
19. Vào `Batch Adjustments` tạo adjustment mẫu
20. Vào `Control > Ca làm việc`
21. Chọn tháng hiện tại, chỉnh thử một vài ca và đơn giá rồi lưu bằng `admin`
22. Nhập thử `phụ cấp`, `đi trễ`, `về sớm` cho ít nhất 1 nhân viên
23. Bấm `In bảng lương` hoặc `Xuất Excel`
24. Chuyển trạng thái sang `LOCKED` và xác nhận grid chấm công bị khóa
25. Chuyển lại trạng thái `PUBLISHED` để xác nhận admin có thể mở khóa tạm thời khi cần
...
### Route Dashboard hoặc Admin Recipes tự chuyển hướng

- Đây là hành vi hiện tại của web UI.
- Dùng `Scan Logs`, `Kho nguyên liệu`, `Ca làm việc` và `Admin reports` cho vận hành hằng ngày.
- Các API POS/reconciliation/anomaly vẫn còn ở backend nếu cần tích hợp riêng.
...
15. Mở `Control > Kho nguyên liệu`
16. Kiểm tra bảng đã cộng đúng số lượng vào ngày/ca vừa quét
17. Mở `Scan logs` và `Kho nguyên liệu` để xem dữ liệu cập nhật
18. Mở `Control > Ca làm việc`
19. Kiểm tra bảng chấm công tháng hiện tại, nhập thử `phụ cấp / đi trễ / về sớm`
20. Thử `In bảng lương` hoặc `Xuất Excel`
21. Nếu cần, chuyển trạng thái `LOCKED` rồi mở lại bằng `admin` để xác nhận cơ chế khóa tạm
